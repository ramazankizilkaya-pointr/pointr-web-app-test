...
When("I open demo app in a browser", () => {
  ...
  cy.visit("https://pointr-sample-web-app.pointr.cloud/");
...
When('I see search box, magnify icon and placeholder are visible', function(){
  cy.get("#search-box >input").should(
    "have.attr",
    "placeholder",
    "Search For Places"
  ).and('be.visible');
...
Then('I should verify category list is visible with {string} options inside', function(count_of_options){
  cy.get('#category-list-container > .category-item').should('be.visible').and('have.length', count_of_options)
})
...
When('I click on each letter in order', function(){
  cy.get('#letterList > .letter-container.clickable').each(($element) => {
    cy.wrap($element).click().then(($element)=>{
          cy.wait(500)
...
When('I click on each category option in order', function(){
  cy.get('div#category-list-container > .category-item').then(($el) => { 
    ...
    for (let index = 1; index <= categoryCount; index++) {
      cy.xpath(`//div[@id='category-list-container']/div[@class= 'category-item'][${index}]`).then(($el2)=>{
        let categoryText = $el2.text()
        cy.wrap($el2).click()
        cy.get('#title-box').should('be.visible').and('have.text', categoryText)
        cy.xpath("//div[@id='letterList' and count(.//div[contains(@class, 'letter-container') and contains(@class, 'clickable')])<26]").should('be.visible')
        cy.wait(500)
...
When('I click on {string} from letter list', (mychar)=>{
  cy.xpath(`//div[@id= 'letterList']//div[text()='${mychar}']`).click().then(($el)=>{
...
When('I scroll search results {string}', (direction)=>{
  if(direction=="down"){
    cy.get('#lettered-list-container > #scrollable-list').scrollTo('bottom', { easing: 'linear', duration: 5000 })
  }
  if(direction=="up"){
    cy.get('#lettered-list-container > #scrollable-list').scrollTo('top', { easing: 'linear', duration: 5000 })
  }
  cy.wait(500)
})
...
When('I click on random search result', ()=>{
  cy.xpath(`(//div[@id='scrollable-list']//div[@data-v-407f8fde and not(contains(@class, 'clickable')) and not(./div[@class='clickable-item-container-expandable'])]//h3[starts-with(text(), '${activeLetter}')])[1]`).click()
})
...
Then('Set up a route modal should be displayed', ()=>{
  cy.xpath("//div[@id='navigation']//div[@id='navigation-title' and text()='Set up a route']").should('be.visible')
})
...
When('I expand a search group', ()=>{
  cy.xpath(`(//div[@id='scrollable-list']//div[@data-v-407f8fde and ./div[@class='clickable-item-container-expandable']])[1]`).click()
})

When('I click on random search result from the expanded search group', ()=>{
  cy.xpath(`(//div[@id='scrollable-list']//div[@data-v-407f8fde and ./div[contains(@class,'clickable-item-container-expandable') and contains(@class, 'expanded')]]//h3)[2]`).click()
})